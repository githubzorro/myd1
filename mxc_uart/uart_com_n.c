...
int dvr_com = FALSE;
enum all_state dvr_s = AS_ON;			/*DVR record*/
enum all_state gsr_s = AS_ON;			/* G-sensor */
enum all_state egy_s = AS_ON;			/*Emergency*/
...
unsigned int available_disk = 8120;
unsigned int total_disk = 8120;

unsigned char dvr_period = 5;
unsigned char dvr_mode = 4;

unsigned char egy_record = 0;
...
msg_unpack(unsigned char buff[], int *rsp_cmd, unsigned char *errvalue)
{
	...
	*rsp_cmd = msg_id+1;
	switch (msg_id){
		...
		case 0x38:
		{
			unsigned short int video_id = buff[4];
			video_id<<8;
			video_id  |= buff[5];
			//replay_video(video_id);
			*errvalue = 0;
			printf("\nzorro, msg req replay video, replay video id   = %ul, \n", video_id);
		}
			break;
		case 0x3A:
		...
		case 0x40:
			*errvalue = 0;
			if (buff[4] & 0x80){
				dvr_period =  (buff[4] & 0xEF);
				printf("\nzorro, msg req set parameter, dvr period = %d, \n",  dvr_period);
			}
			if (buff[5] & 0x80){
				unsigned char mode = buff[5] & 0xEF;
				if ((mode == 1) ||(mode ==4))
					dvr_mode = mode;
				else
					*errvalue = 1;
