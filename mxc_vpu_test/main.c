...
parse_main_args(int argc, char *argv[])
{
	...
	do {
		...
		switch (opt)
		{
		case 'D':			/*程序参数为解码，把解码的参数复制到input_arg.line*/
			...
			instance++;
			break;
		case 'E':		/*程序参数为编码，把编码参数复制到input_arg.line */
	...
	printf("\nliuxu, 6/3/2014, MAX_NUM_INSTANCE=%d,!!!\n",MAX_NUM_INSTANCE);
	optind = 1;
	return status;
}
...
#endif
{
	int err, nargc, i, j, ret = 0;
	char *pargv[32] = {0}, *dbg_env;
	pthread_t sigtid, keytid;
	vpu_versioninfo ver;
	int ret_thr;
	char *ctimes;
	char rename[50];
	...
	err = parse_main_args(argc, argv);	/*解析程序参数*/
	if (err) {
		goto usage;
	}
	...
	framebuf_init();	/*初始化framebuf，把指针池里的指针一一指向framebuf*/

#ifdef COMMON_INIT
	err = vpu_Init(NULL);	/*初始化vpu*/
	if (err) {
		err_msg("VPU Init Failure.\n");
		return -1;
	}

	err = vpu_GetVersionInfo(&ver);	/*获得vpu版本*/
	if (err) {
...
#ifdef KEY_ENABLE
	pthread_create(&keytid, NULL, (void *)&key_thread, NULL);
#endif

	printf("\nzorro, 5/27/2015, instance=%d!!!\n", instance);
...
Circu_record:
	while(enable_record%2)
	{
		...
		if (instance > 1) {
			for (i = 0; i < instance; i++) {
				...
				/*rename the output file with system time*/
				tv_sec = time(NULL);
				ctimes = ctime(&tv_sec);
				rename[0] = '.';
				rename[CTIME_SZIE+1] = '\0';
				strncpy(&rename[1], ctimes, CTIME_SZIE); 		
				for (j=0; rename[j]!='\0'; j++){
					if (rename[j]== ' '){
						rename[j] = '-';
						if (rename[j+1] == ' ')
							rename[j+1] = '0';
				...
				strncat(input_arg[i].cmd.output, rename, CTIME_SZIE+2);
				printf("\nzorro,  output:%s.  j = %d!!!\n\n",input_arg[i].cmd.output, j);
				...
				printf("\nzorro, 5/27/2015, input_arg[i].tid=%lu!!!\n", input_arg[i].tid);

			}
		} else {
			if (using_config_file == 0) {
				get_arg(input_arg[0].line, &nargc, pargv);	 	/*get param from input_arg[]*/
				err = parse_args(nargc, pargv, 0);			/*Analytical command*/
				if (err) {
					vpu_UnInit();
					goto usage;
				}
			...
			ctimes = ctime(&tv_sec);
			name_temp[0] = '.';
			name_temp[CTIME_SZIE+1] = '\0';
			strncpy(&name_temp[1], ctimes, CTIME_SZIE); 
			...
			strncat(input_arg[0].cmd.output, name_temp, CTIME_SZIE+2);
			printf("\nzorro,  output:%s.  j = %d!!!\n\n",input_arg[0].cmd.output, j);
			
			if (check_params(&input_arg[0].cmd, input_arg[0].mode) == 0) {	/*Fill the default parameter*/
				if (open_files(&input_arg[0].cmd) == 0) {
					...
					if (input_arg[0].mode == ENCODE)
					    create_list(list_head, input_arg[0].cmd.output, 0, 0);
				} else {
					ret = -1;
				}
			} else {
		...
		if (instance > 1) {
			for (i = 0; i < instance; i++) {
				if (input_arg[i].tid != 0) {
					...
					if (input_arg[i].mode == ENCODE)
					    create_list(list_head, input_arg[i].cmd.output, 0, 0);
				}
			}
		}
	...
	if (keytid !=0){		
		if (pthread_cancel(keytid) != 0){
			pthread_join(keytid, (void *)&ret_thr);
				if (ret_thr)
					ret = -1;
		}
	}
