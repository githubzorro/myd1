...
struct instance_priv ins_priv[INSTANCE_NUM];
...
//int g_display_top = 0;	//display is diffrent between each instance
...
//			buf.m.userptr = (unsigned long) ins_priv[instns].cap_buffers[i].offset;
...
int
v4l_capture_setup(struct encode *enc, int width, int height, int fps, int instns)
{
	...
	printf("v4l_capture_setup:instns = %d,  display left = %d, top = %d, width = %d, height = %d.\r\n\n",instns, ins_priv[instns].g_display_left, ins_priv[instns].g_display_top, g_display_width, g_display_height);
...
{	
	...
	draw_image_to_framebuffer(ins_priv[instns].g2d_buffers[buf->index], g_in_width, g_in_height, g_g2d_fmt, &g_screen_info, ins_priv[instns].g_display_left, ins_priv[instns].g_display_top, g_display_width, g_display_height, 0, G2D_ROTATION_0);
}
