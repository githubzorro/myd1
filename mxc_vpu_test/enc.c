...
//extern struct g2d_buf *g2d_buffers[];  //qiang_debug added
...
put_mp4header:
		...
		if (enc->ringBufferEnable == 0 ) {
		 ...
		 /*printf:mvc_extension=0,  enc->mvc_paraset_refresh_en=0, enc->ringBufferEnable = 0 */
...
int
encoder_allocate_framebuffer(struct encode *enc, int instns)
{
	EncHandle handle = enc->handle;
	...
	enc->totalfb = totalfb = minfbcount + extrafbcount + srcfbcount; /*帧的最大数*/

	printf("\nzorro, 5/27/2015, minfbcount=%d, extrafbcount=%d, srcfbcount=%d!!!\n", minfbcount, extrafbcount, srcfbcount);
	//Print out the results  minfbcount=2，extrafbcount=2，srcfbcount=1
	
	/* last framebuffer is used as src frame in the test */
	enc->src_fbid = src_fbid = totalfb - 1;						/*源帧的id*/

	fb = enc->fb = calloc(totalfb, sizeof(FrameBuffer));				/*分配N个帧buffer*/
	if (fb == NULL) {
	...
	pfbpool = enc->pfbpool = calloc(totalfb,
					sizeof(struct frame_buf *));				/*分配N个帧buffer指针*/
	if (pfbpool == NULL) {
	...
	printf("\nzorro, 5/27/2015, enc->cmdl->mapType=%d!!!\n\n",enc->cmdl->mapType);
	//enc->cmdl->mapType=0!
	...
	ret = vpu_EncRegisterFrameBuffer(handle, fb, minfbcount, enc_stride, src_stride,	/*注册之前申请到的buffer*/
	...
	if (enc->cmdl->src_scheme == PATH_V4L2) {		/*设置v4l2 */
		ret = v4l_capture_setup(enc, enc->src_picwidth, enc->src_picheight, enc->cmdl->fps, instns);
		if (ret < 0) {
			goto err1;
		}
	} else {
...
encoder_start(struct encode *enc)
{
	...
	if (src_scheme == PATH_V4L2) {
		ret = v4l_start_capturing(enc->cmdl->instns);			/*启动v4l2 捕获*/
		if (ret < 0) {
			return -1;
		}
	...
	printf("\nzorro, 6/1/2015, enc->ringBufferEnable =%d,!!!\n\n", enc->ringBufferEnable);
	//printed value, ringBufferEnable = 0
	...
	while (1) {
		if (src_scheme == PATH_V4L2) {
			ret = v4l_get_capture_data(&v4l2_buf, enc->cmdl->instns);	/*get frame data from v4l2*/
			if (ret < 0) {
				goto err2;
			}
			...
			fb[src_fbid].bufY = ins_priv[0].g2d_buffers[v4l2_buf.index]->buf_paddr;  //qiang_debug modified
		...
		ret = vpu_EncStartOneFrame(handle, &enc_param);		/*start encode one frame*/
		if (ret != RETCODE_SUCCESS) {
		...
		while (vpu_IsBusy()) {
			...
			if (enc->ringBufferEnable == 1) {
				ret = enc_readbs_ring_buffer(handle, enc->cmdl,		/*get the encoded value and write to a file*/
			...
			if (loop_id == 20) {
				ret = vpu_SWReset(handle, 0);		/* reset vpu*/
				return -1;
			}
			loop_id ++;
		}
		...
		ret = vpu_EncGetOutputInfo(handle, &outinfo);	   /*获取编码器生成的数据的参数*/
		if (ret != RETCODE_SUCCESS) {
		...
		if (src_scheme == PATH_V4L2) {
			v4l_put_capture_data(&v4l2_buf, enc->cmdl->instns);	/*把数据帧放入缓冲队列。可能是显示用*/
		}
		...
		if (enc->ringBufferEnable == 0) {
			ret = enc_readbs_reset_buffer(enc, outinfo.bitstreamBuffer, outinfo.bitstreamSize);	/*把获取的数据流写入文件*/
			if (ret < 0) {
...
encoder_open(struct encode *enc)
{
	...
	if (enc->cmdl->width && enc->cmdl->height) {
		enc->src_picwidth = enc->cmdl->width;			/*编码源图宽*/
		enc->src_picheight = enc->cmdl->height;		/*编码源图高*/
	}

	/* enc_width and enc_height in command line means encoder output size */
	if (enc->cmdl->enc_width && enc->cmdl->enc_height) {
		enc->enc_picwidth = enc->cmdl->enc_width;		/*编码目标图宽*/
		enc->enc_picheight = enc->cmdl->enc_height;	/*编码目标图高*/
	} else {
	...
	if (enc->cmdl->rot_angle == 90 || enc->cmdl->rot_angle == 270) {		/*是否进行旋转*/
	...
	if(!cpu_is_mx6x() &&  enc->cmdl->format == STD_MJPG )		/*我们是STD_AVC*/
	{
	...
	ret = vpu_EncOpen(&handle, &encop);		/*打开一个编码实例*/
	if (ret != RETCODE_SUCCESS) {
		if (enc->cmdl->format == STD_MJPG) {
			free(qMatTable);
			free(huffTable);
...
encode_test(void *arg)
{
	...
	if (cmdl->dst_scheme == PATH_NET) {		/*是否是网络传输编码文件*/
		sleep(10);
	}

	/* allocate memory for must remember stuff */
	enc = (struct encode *)calloc(1, sizeof(struct encode));	/*分配一个encod结构体*/
	if (enc == NULL) {
	...
	ret = IOGetPhyMem(&mem_desc);				/* 从vpu驱动中申请一段物理内存*/
	if (ret) {
	...
	enc->virt_bsbuf_addr = IOGetVirtMem(&mem_desc);	/*把申请的物理内存映射进应用层*/
	...
	ret = encoder_allocate_framebuffer(enc, cmdl->instns);
	if (ret)
		goto err1;
	...
	encoder_free_framebuffer(enc);			/*释放buffer */
err1:
	/* close the encoder */
	encoder_close(enc);						/*关闭编码*/
err:
	if (cpu_is_mx6x() && cmdl->format == STD_MPEG4 && enc->mp4_dataPartitionEnable) {
